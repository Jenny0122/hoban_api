...
import com.wisenut.ebk.spring.vo.DocumentSearchVo;
...
import com.wisenut.ebk.spring.vo.FolderSearchVo;
import com.wisenut.ebk.spring.vo.SecurityVo;
...
=======
	...
	public FileSearch searchFileTotalListByCategory(Map<String, String> params) throws MissingArgumentException {
		...
		log.info("**********************************");
		params.keySet().stream().forEach(key -> log.info(" - " + key + ": {}", params.get(key)));
		log.info("**********************************");
...
//			ret = search.w3SetFilterQuery(COLLECTION, sb.toString());
			filterQueryBuilder.append(sb.toString().subSequence(0, sb.toString().length() - 1));
			filterQueryBuilder.append(")");
			filterQueryBuilder.append(" ");
		} else {
			throw new MissingArgumentException("aclFilterInfos는 \'필수\'값 입니다.");
...
//			ret = search.w3SetFilterQuery(COLLECTION, "<FOLDERFULLPATHOID:substring:"+ folderOid + ">");
		...
		log.info("filterQuery: {}", filterQueryBuilder.toString().trim());
		log.info("collectionQuery: {}", collectionQueryBuilder.toString().trim());
		ret = search.w3SetFilterQuery(COLLECTION, filterQueryBuilder.toString().trim());
		ret = search.w3SetCollectionQuery(COLLECTION, collectionQueryBuilder.toString().trim());
	...
	public DocumentSearch searchDocumentTotalListByCategory(Map<String, String> params) throws MissingArgumentException {
		...
		return DocumentSearch.builder().Collection(COLLECTION).TotalCount(totalCount).Count(resultCount).Result(list)
				.build();
	}
...
//			ret = search.w3SetFilterQuery(COLLECTION, sb.toString());
			filterQueryBuilder.append(sb.toString().substring(0, sb.toString().length() - 1));
...
//			ret = search.w3SetCollectionQuery(COLLECTION, "<FOLDERFULLPATHOID:contains:"+ folderOid + ">");
		...
		return FolderSearch.builder().Collection(COLLECTION).TotalCount(totalCount).Count(resultCount).Result(list)
				.build();
	...
	public SearchPersonalDTO searchPersonalDataTotalListByCategory(Map<String, String> params) throws MissingArgumentException {
		...
		for (int i = 0; i < resultCount; i++) {
			...
			for(String item: aliasArr) {
				...
				switch(securityInfo) {
					case "JUMIN": securityInfo = "주민등록번호"; break;
					case "PHONE": securityInfo = "휴대폰번호"; break;
					case "DRIVE": securityInfo = "운전면허번호"; break;
					case "PASS_PORT": securityInfo = "여권번호"; break;
					case "CREDIT": securityInfo = "카드번호"; break;
					case "ACCOUNT": securityInfo = "계좌번호"; break;
					case "HEALTH": securityInfo = "건강보험번호"; break;
				}
				
				security.add(SecurityVo.builder().securityInfo(securityInfo).securityCount(securityCount).build());
			}
		...
		FileSearch file = FileSearch.builder().Collection(COLLECTION).TotalCount(totalCount).Count(resultCount).Result(list)
		.build();
		
		SearchPersonalDTOBuilder dtoBuilder = SearchPersonalDTO.builder();
		List<Object> data = new ArrayList<>();
		data.add(file);
		SearchPersonalDTO dto = dtoBuilder.data(data).query(query).customCategoryMap(tagCountMap).build();
